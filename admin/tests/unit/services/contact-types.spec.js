const { CONTACT_TYPES, DOC_TYPES: docType } = require('@medic/constants');
...
describe('ContactTypes service', () => {
  ...
  describe('includes', () => {
    ...
    it('returns true for contact type', () => {
      chai.expect(service.includes({ type: docType.CONTACT})).to.equal(true);
    });
  ...
  describe('getTypeId', () => {
    it('should return the type id of the provided contact', () => {
      ...
      chai.expect(service.getTypeId({ type: docType.CONTACT, contact_type: 'something' })).to.equal('something');
    });
